...
function CandidateFeedbackDialog({candidate}) {
  ...
  <DialogTrigger asChild>
    <Button variant="outline" className='cursor-pointer text-primary'>View Report</Button>
  </DialogTrigger>
  <DialogContent>
    <DialogHeader>
            ...
            <div className='mt-5'>
                ...
                <div className='p-5 bg-secondary my-3 rounded-md'>
                  {Array.isArray(feedback?.summary)
                    ? feedback.summary.map((summary, index) => <p key={index}>{summary}</p>)
                    : typeof feedback?.summary === 'string'
                      ? feedback.summary.split('\n').map((line, idx) => <p key={idx}>{line}</p>)
                      : null}
                </div>
            </div>
            <div className={`p-5 mt-8 flex items-center justify-between rounded-md ${feedback?.recommendation=='No'?'bg-red-100':'bg-green-100'}`}>
                <div>
                <h2 className={`${feedback?.recommendation=='No'?'text-red-700':'text-green-700'}`}>Recommendation Msg:</h2>
                <p className={`${feedback?.recommendation=='No'?'text-red-500':'text-green-400'}`}>{feedback?.recommendationMsg}</p>
                </div>
